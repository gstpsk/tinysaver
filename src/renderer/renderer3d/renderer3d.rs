...
impl Renderer3D {
    ...
    fn create_texture_bind_group(device: &wgpu::Device, bind_group_layout: &wgpu::BindGroupLayout, texture_view: wgpu::TextureView, sampler: wgpu::Sampler) -> wgpu::BindGroup {
        device.create_bind_group(&wgpu::BindGroupDescriptor {
    ...
    fn create_render_pipeline_layout(device: &wgpu::Device, mvp_bind_group_layout: &wgpu::BindGroupLayout) -> wgpu::PipelineLayout {
        device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("epic render pipeline layout"),
            bind_group_layouts: &[Some(mvp_bind_group_layout)],
            immediate_size: 0,
        })
    }
    ...
    pub fn new(device: &wgpu::Device, surface_format: wgpu::TextureFormat, surface_width: u32, surface_height: u32) -> Self {
        ...
        let transform_bind_group_layout = Self::create_transform_bind_group_layout(device);
        let transform_bind_group = Self::create_transform_bind_group(device, &transform_bind_group_layout, &transform_buffer);
                
        let render_pipeline_layout = Self::create_render_pipeline_layout(device, &transform_bind_group_layout);
        
        let solid_render_pipeline = Self::create_render_pipeline(device, &render_pipeline_layout, &shader, "fs_main", surface_format, true);

        let wireframe_render_pipeline = Self::create_render_pipeline(device, &render_pipeline_layout, &shader, "fs_main", surface_format, false);
    ...
    pub fn load_textures(device: &wgpu::Device, queue: &wgpu::Queue) {
        let image = image::open("arch25percent.png").expect("Failed to load image");
        let diffuse_texture = diffuse_texture::create_diffuse_texture(device, queue, Some("epic texture"), image);

    }

    // expects a single type of instances
    pub fn render_mesh(&self, encoder: &mut wgpu::CommandEncoder, target: &wgpu::TextureView, depth_view: &wgpu::TextureView, mesh: &GpuMesh, wireframe_mode: bool) {
